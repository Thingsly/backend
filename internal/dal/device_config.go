...
// Get device configuration dropdown menu.
...
func BatchUpdateDeviceConfig(req *model.BatchUpdateDeviceConfigReq) error {
	...
	if len(devices) > 0 {
		return errors.Errorf("Device ID: %s is already bound to another configuration.", devices)
	}
	t := time.Now().UTC()
...
// Query the number of configuration templates to be associated based on the feature template ID.
